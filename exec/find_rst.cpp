...
JSAMPLE *in_image_buffer;   /* Points to large array of R,G,B-order data */
...
GLOBAL(void)
write_JPEG_file(const char *filename, int quality, unsigned int restart_interval, int restart_in_rows)
{
  ...
  printf("Restart interval %d, Restart row %d\n", restart_interval, restart_in_rows);
  ...
  while (cinfo.next_scanline < cinfo.image_height) {
     ...
     */
    row_pointer[0] = &in_image_buffer[cinfo.next_scanline * row_stride];
...
GLOBAL(int)
read_JPEG_file(const char *in_filename)
{
  ...
  return do_read_JPEG_file(&cinfo, in_filename);
}
...
METHODDEF(int)
do_read_JPEG_file(struct jpeg_decompress_struct *cinfo, const char *in_filename)
{
   ...
   * loop counter, so that we don't have to keep track ourselves.
   */
  long row_size = sizeof(JSAMPLE) * row_stride;
  long write_bytes = 0;
  while (cinfo->output_scanline < cinfo->output_height) {
    ...
    memcpy(out_image_buffer + write_bytes, buffer[0], row_size);
    write_bytes += row_size;
  }
...
int main(void) {
  ...
  for (const auto & entry : std::filesystem::directory_iterator("/home/kichang/Downloads/Flickr2K/Flickr2K_HR/")) {
    ...
    write_JPEG_file(enc_out_filename.c_str(), 75, 1, 0);
    ...
    int img_size;
    int rst_count = 0;
    std::ifstream enc_file(enc_out_filename.c_str());
    if (enc_file.is_open()) {
        enc_file.seekg(0, std::ios::end);
        img_size = enc_file.tellg();
        img = new char[img_size];
        enc_file.seekg(0, std::ios::beg);
        enc_file.read(img, img_size);
        enc_file.close();
    }
    ...
    time_us = std::chrono::duration_cast<std::chrono::microseconds>(chrono_clock::now()-start).count();

    std::cout << "Total RST: " << rst_count << ", " << "scan time: " << time_us << std::endl;
