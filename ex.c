...
GLOBAL(void)
write_JPEG_file(char *filename, int quality, unsigned int restart_interval, int restart_in_rows)
{
...
int main(void) {
  image_width = 1920;
  image_height = 1080;
  image_buffer = malloc(sizeof(JSAMPLE) * image_width * image_height * 3); // rgb
  for (int r = 0; r < image_height; r++) {
    for (int c = 0; c < image_width; c++) {
      image_buffer[r * image_width * 3 + c * 3 + 0] = (r*5) % 255;
      image_buffer[r * image_width * 3 + c * 3 + 1] = (r*5) % 255;
      image_buffer[r * image_width * 3 + c * 3 + 2] = (r*5) % 255;
    }
  }
  ...
  write_JPEG_file("test.jpg", 75, 1, 0);
...
METHODDEF(void) put_scanline_someplace(JSAMPROW row, int i) {
